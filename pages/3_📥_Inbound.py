# pages/3_📥_Inbound.py - Inbound Module Main Page
...
# Import from inbound-specific utils
from utils.inbound.data_loader import InboundDataLoader
...
# Page config
st.set_page_config(
    page_title="Inbound Logistics - iSCM",
    page_icon="📥",
    layout="wide",
...
# CSS styling (adapted from original app.py)
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem;
...
with st.sidebar:
    ...
    module = st.radio(
        "Select Module",
        ["📊 Dashboard", "🔍 PO Tracking", "📦 CAN Management", "📧 Email Notifications", "📈 Analytics & Reports"],
        key="inbound_module_selection"
    )
    ...
    st.caption(f"Inbound v{APP_VERSION}")

# Route to appropriate module
if module == "🔍 PO Tracking":
    # Import and run PO tracking module
    try:
        from modules.inbound.po_tracking import show_po_tracking
        show_po_tracking()
    except ImportError:
        st.error("PO Tracking module not found. Please ensure modules/inbound/po_tracking.py exists.")
    except Exception as e:
        st.error(f"Error loading PO Tracking: {str(e)}")
        logger.error(f"PO Tracking error: {e}")

elif module == "📦 CAN Management":
    # Import and run CAN management module
    try:
        from modules.inbound.can_management import show_can_management
        show_can_management()
    except ImportError:
        st.error("CAN Management module not found. Please ensure modules/inbound/can_management.py exists.")
    except Exception as e:
        st.error(f"Error loading CAN Management: {str(e)}")
        logger.error(f"CAN Management error: {e}")

elif module == "📧 Email Notifications":
    # Import and run email notifications module
    try:
        from modules.inbound.email_notifications import show_email_notifications
        show_email_notifications()
    except ImportError as e:
        st.error(f"Email Notifications import error: {str(e)}")
        import traceback
        st.code(traceback.format_exc())
    except Exception as e:
        st.error(f"Error loading Email Notifications: {str(e)}")
        logger.error(f"Email Notifications error: {e}")

elif module == "📈 Analytics & Reports":
    # Import and run analytics module
    try:
        from modules.inbound.analytics_reports import show_analytics_reports
        show_analytics_reports()
    except ImportError:
        st.error("Analytics & Reports module not found. Please ensure modules/inbound/analytics_reports.py exists.")
    except Exception as e:
        st.error(f"Error loading Analytics & Reports: {str(e)}")
        logger.error(f"Analytics & Reports error: {e}")

else:  # Dashboard (default)
    # Show main dashboard (adapted from original app.py)
...
# Footer info
st.markdown("---")
st.caption(f"Inbound Logistics Module v{APP_VERSION} | Part of iSCM Dashboard")